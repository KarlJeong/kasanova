...
- 결과 키 컬럼 선택 — 여러 서브쿼리 결과를 공통 키로 결합해야 하므로 키 의미·타입이 일관되어야 한다:
  · "회원 식별값"·"투자자 식별값" 등 회원을 식별하는 키가 필요하면 팩트 테이블의 `member_id`(int FK)를 선택하라. `kasa_member.uuid`는 결과 키로 선택하지 않는다. 불가피하게 `kasa_member`를 조인해야 하는 경우에만 `kasa_member.id`를 사용하라.
  · "DABS 식별값"·"종목 식별값" 등 DABS를 식별하는 키가 필요하면 `dabs_code`(또는 `kasa_ledger_dabs.code`, char)를 선택하라. 내부 int id는 결과 키로 선택하지 않는다.
