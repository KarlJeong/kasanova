...
class QueryPlan(BaseModel):
    ...
    key_column: str | None = Field(
        default=None,
        description=(
            "각 sub-query 결과의 결합 키 컬럼명. "
            "사용자 목록 질문은 보통 'member_id'."
        ),
    )
...
공통 키 컬럼(예: member_id)으로 set 연산하여 결합할 수 있게 만든다.
...
[분해 규칙]
- 각 sub-question은 **하나의 조건만** 담아야 한다. "X한 사용자 member_id 목록" 형태.
- sub-question 본문은 한국어 자연어로 자연스럽게 쓰되, 결과로 어떤 키 컬럼을 반환해야 하는지 명시.
- 결합 키는 모든 sub-question에서 동일해야 한다 (보통 member_id).
- combine은 질문의 논리 접속사로 결정: "그리고/하고/이며/이면서" → intersect, "또는/이거나" → union, "했지만 ~ 안 한" → difference.
